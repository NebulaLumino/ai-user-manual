#!/usr/bin/env node
// Builder script — creates a single AI Creative Arts app

const APP_NAME = process.argv[2];
const HSL = process.argv[3]; // e.g. "145,60,50"
const PROMPT = process.argv[4]; // the AI prompt template

const [h, s, l] = HSL.split(',').map(Number);

const systemPrompt = PROMPT || `You are an expert assistant for ${APP_NAME}. Based on the user's input, generate high-quality creative content. Be specific, detailed, and actionable.`;
...
export default function App() {
  ...
  const accentColor = "hsl(${h}deg, ${s}%, ${l}%)";
  const accentRgb = `hsl(${h}, ${s}%, ${l}%)`;
    ...
    <main className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-gray-900 text-white flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-2xl">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-bold mb-2" style={{ color: "${accentRgb}" }}>
            ${PROMPT.split(' ')[0] || '✨'} ${APP_NAME.replace(/-/g, ' ').replace(/\\b\\w/g, c => c.toUpperCase())}
          </h1>
        ...
        <div className="bg-gray-900/80 border border-gray-800 rounded-2xl p-6 mb-6">
          ...
          <textarea
            ...
            style={{ "--tw-ring-color": "${accentRgb}" } as React.CSSProperties}
            rows={5}
...
console.log("Route:", routeContent.substring(0, 100));
